...
### Сжатие рефов перед отправкой (2026-07-04)
Рефы уходят в ТЕЛЕ submit-POST как base64 data-URI (`inputs[].input`), НЕ через
`/upload` (storage-hash от ключа A не виден ключу B → 403; data-URI не зависит от
ключа). Тяжёлый реф (4K/~4МБ) × base64 (+33%) = ~5.5МБ тела на реф; 3 рефа ≈ 16МБ →
на медленном аплоаде gateway FastGen рвёт запрос: `socket.timeout` «write operation
timed out» (наш `timeout=60` покрывает и фазу записи) ИЛИ HTTP 499 «Client Closed
Request». `_file_to_data_uri` (`generator_video_thread.py` и `generator_thread.py` —
ИДЕНТИЧНОЕ тело) жмёт **копию в памяти** (`io.BytesIO`, on-disk реф НЕ трогаем)
**КАЖДЫЙ реф** (2026-07-05: порог убран — при 7-10 рефах даже средние 0.8-1.2МБ
раздували тело; порог 1.5МБ пропускал их сырыми): downscale ≤2048px (ТОЛЬКО вниз) +
`convert RGB` + JPEG `quality` 90→80 до ~1МБ, затем шлёт версию, что **ЛЕГЧЕ**
(`len(compressed) < len(raw)` → сжатая, иначе сырой оригинал — защита от раздувания
уже-оптимального мелкого рефа). Весь блок в `try/except` → ошибка PIL = фолбэк на
сырой файл (генерацию не роняет).
Замер: 4.15МБ реф → ~1.08МБ тела (×5). PIL — ленивый импорт (не тянется в frozen на
старте). Лимита разрешения рефа у FastGen в коде/доках нет (только `_max_refs` —
