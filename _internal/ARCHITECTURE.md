...
условие `max(side) > 2048px OR вес > 1.5МБ` → `thumbnail(2048, LANCZOS)` + `convert
RGB` + JPEG `quality` 90→80 до ~1МБ; иначе (мелкий лёгкий) — уходит сырым как есть.
Весь блок в `try/except` → ошибка PIL = фолбэк на сырой файл (генерацию не роняет).
