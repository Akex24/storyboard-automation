...
def main():
    ...
    try:
        ...
        pass
    # 2026-07-05: сброс языка UI на РУССКИЙ при КАЖДОЙ пересборке. Ключим на build-stamp
    # из бандла (build_stamp.txt — печётся StoryboardStudio.spec'ом на Mac И Win, уникален
    # на каждый билд), а НЕ на app_version: локальная пересборка версию не бампает, старый
    # version-ключ на неё не срабатывал. Новый стемп → один раз ставим ru + запоминаем.
    # Тот же стемп (обычный рестарт той же сборки) → уважаем выбор юзера (uk остаётся uk).
    # Стемпа нет (dev-запуск из исходников) → не трогаем. Namespace QSettings как у
    # get_lang/set_lang. Сбой миграции старт не роняет.
    try:
        _stamp_base = Path(sys._MEIPASS) if hasattr(sys, "_MEIPASS") else Path(__file__).parent
        _stamp_file = _stamp_base / "build_stamp.txt"
        _cur_stamp = _stamp_file.read_text(encoding="utf-8").strip() if _stamp_file.exists() else ""
        if _cur_stamp:
            _lang_qs = QSettings(APP_ORG, APP_NAME)
            if _cur_stamp != _lang_qs.value("lang_reset_build", "", type=str):
                _lang_qs.setValue("ui_lang", "ru")
                _lang_qs.setValue("lang_reset_build", _cur_stamp)
                sys.stderr.write(f"[migrate] ui_lang -> ru (new build stamp {_cur_stamp})\n")
    except Exception:
        pass
    install_theme_runtime()
