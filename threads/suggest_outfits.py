...
def build_outfit_prompt(name: str, scenario_text: str = "",
    ...
    return (
        ...
        f"{avoid_block}"
        f"ЗАДАЧА:\n\n"
        f"ШАГ 1 (ОБЯЗАТЕЛЬНО ПЕРЕД ВЫБОРОМ ВАРИАНТОВ):\n"
        f"Найди в СЦЕНАРИИ ЭПИЗОДА (фрагмент выше) ПЕРВУЮ по хронологии "
        f"сцену где появляется «{name}». Учитывай контекст этой сцены — "
        f"локация, время суток, действие персонажа, эмоциональный регистр. "
        f"Все 3 варианта одежды должны подходить ИМЕННО к этой первой сцене. "
        f"Не пытайся охватить разные аутфиты для разных сцен эпизода — юзер "
        f"сам перегенерирует или добавит варианты вручную после первой "
        f"генерации через вкладку «Актёры».\n"
        f"Если в КОНТЕКСТЕ ОТ АГЕНТА выше (priority #1) явно указана одежда "
        f"для конкретных сцен — она OVERRIDE'ит вывод из первой сцены: следуй "
        f"контексту агента.\n\n"
        f"ШАГ 2: Дай ровно 3 варианта одежды персонажа «{name}» подходящих "
        f"под контекст первой сцены (или под override от агента). Каждый "
        f"вариант — одна короткая фраза (на русском).\n\n"
