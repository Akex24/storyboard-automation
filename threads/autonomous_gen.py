...
def build_autonomous_prompt(gen_type: str, name: str, description: str,
    ...
    if gen_type == "location":
        pipeline_steps = (
            ...
            f" 'suburban / wooden dacha / weathered / Russian provincial'"
            f" дефолт независимо от того что в description.\n"
            ...
            f"      • Плохо: 'private suburban house, wooden picket"
            f" fence, slightly worn dacha aesthetic, Russian provincial"
            f" atmosphere'\n"
            f"     Аналогично для бедных/criminal/historical сериалов —\n"
            f"     передавай тот контекст что в description (grungy /\n"
            f"     working-class / 90s soviet / Victorian / etc).\n"
