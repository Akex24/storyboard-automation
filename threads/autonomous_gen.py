...
def build_autonomous_prompt(gen_type: str, name: str, description: str,
    ...
    if gen_type == "location":
        pipeline_steps = (
            ...
            f"`python3 pipeline.py generate {name} \"<промпт>\" --force`. "
            f"🔴 Флаг `--force` ОБЯЗАТЕЛЕН: юзер кликнул «🎨 Сгенерировать» "
            f"в Studio → ожидает НОВУЮ картинку, даже если файл уже есть. "
            f"Без `--force` pipeline.py выйдет с «Already exists» и "
            f"генерация фактически не произойдёт (старый файл останется). "
        ...
        pipeline_steps = (
            ...
            f"`python3 pipeline.py generate {name} \"<промпт>\" --force --kind=object`. "
            f"🔴 Флаги ОБЯЗАТЕЛЬНЫ:\n"
            f"     • `--force` — юзер кликнул «🎨 Сгенерировать» в Studio "
            f"→ ожидает НОВУЮ картинку.\n"
            f"     • `--kind=object` — pipeline сохранит .jpg И _prompt.txt "
            f"СРАЗУ в refs/objects/ (раньше всё шло в refs/locations/, "
            f"приходилось вручную перемещать .jpg).\n"
