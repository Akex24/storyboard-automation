...
class CameraLabView(QWidget):
    ...
    def _run_generation(self) -> None:
        ...
        # 2026-07-03 (семантика ползунка ЗАФИКСИРОВАНА): 0 = МАКСИМАЛЬНО
        # БЛИЗКО (крупный план), 10 = МАКСИМАЛЬНО ДАЛЕКО (общий план).
        # ФИНАЛЬНЫЙ факт (скрин Alex, прямой маппинг): ползунок 0 → fal 0 →
        # ДАЛЬНИЙ план (в полный рост) — значит близкий конец модели = fal 10,
        # description схемы («10=close-up») ВЕРНЫЙ. Маппинг — ИНВЕРСИЯ:
        # ползунок 0 → fal 10 (лицо крупно), ползунок 10 → fal 0 (общий план).
        # Глобус живёт от ПОЛЗУНКА (не от fal): 0 → вплотную, 10 → край.
