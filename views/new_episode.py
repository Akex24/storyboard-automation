...
class NewEpisodeView(QWidget):
    ...
    def _on_run(self):
        ...
        prompt = (
            ...
            "  ЛОКАЦИИ:\n"
            "  - ✗ <name1> (короткое описание места — где это, что происходит) — рефа нет, буду генерировать [[GEN:location:<name1>:<описание>]]\n"
            "  - ✓ <name2> (короткое описание места) — реф есть — сцены X-Y\n"
            "  ОБЪЕКТЫ (правило ≥2 шотов):\n"
            "  - ✗ <obj1> (что это за предмет, для чего нужен) — нужен реф [[GEN:object:<obj1>:<описание>]]\n"
            "  ПЕРСОНАЖИ:\n"
            "  - ✓ <char1> (Имя — короткая роль в эпизоде) — реф есть\n"
            "\n"
            "🔴 ПОСЛЕ «реф есть» — НЕ ПИШИ путь файла в скобках "
            "(`(refs/locations/<name>.jpg)` / `(refs/characters/...)`).\n"
            "  Описание в скобках после slug ВЫШЕ уже даёт юзеру всё нужное —\n"
            "  путь дублирует slug + захламляет строку. ПРАВИЛЬНО:\n"
            "    `- ✓ lawyer_office (кабинет адвоката) — реф есть — сцены 1-4`\n"
            "  НЕПРАВИЛЬНО:\n"
            "    `- ✓ lawyer_office (кабинет адвоката) — реф есть (refs/locations/lawyer_office.jpg) — сцены 1-4`\n"
            "\n"
            ...
            "  • Это нужно чтобы юзер сразу понимал что это за реф БЕЗ открытия\n"
            "    папки. Например: `lawyer_office` — непонятно, а\n"
            "    `lawyer_office (кабинет адвоката где Дэвид получает совет про камеру)`\n"
            "    — мгновенно ясно.\n"
            ...
            "      • Хорошо: `house_corridor (светлый коридор богатого\n"
            "        загородного дома, дорогой паркет, белые стены,\n"
            "        элегантный декор)` → Gemini сделает luxury interior.\n"
            "      • Плохо: `house_corridor (светлый коридор частного\n"
            ...
            "  • Для ПЕРСОНАЖЕЙ — формат `(Имя — короткая роль)`. Имя и роль\n"
            "    разделяются длинным тире `—`. Имя идёт первым, роль — после.\n"
            ...
            "    ЭТОМ эпизоде:\n"
            "      • Хорошо: `david (Дэвид — адвокат, к которому пришёл клиент)`\n"
            "      • Хорошо: `lora (Лора — клиентка, обратилась за защитой)`\n"
            "      • Хорошо: `viktor (Виктор — следователь, ведёт дело)`\n"
            "      • Плохо: `david (Дэвид — главный герой)` — мы не знаем, кто главный\n"
            "      • Плохо: `lora (Лора — протагонистка)` — пустой ярлык\n"
            "    Если из сценария НЕ ясна роль — оставь только Имя без `—`:\n"
            "      • `david (Дэвид)` — допустимо, лучше пусто чем выдумано.\n"
            ...
            "    рефа. Без них picker генерит generic city wear.\n"
            "      • Хорошо: `mark (Марк — любовник Лоры. Сцены 7-8 —\n"
            "        обнажённый торс / простыня по пояс в кровати; сцены\n"
            "        11+ — после ареста, белая футболка под кожаной\n"
            "        курткой)`. Picker предложит контекстные варианты\n"
            "        (бельё, голый торс + спортивные шорты, и т.д.).\n"
            "      • Плохо: `mark (Марк — любовник Лоры, ведёт себя\n"
            ...
            "    ОБЯЗАТЕЛЬНО скопируй эти детали в описание. Если\n"
            "    сценарий молчит про одежду конкретного героя — оставь\n"
            "    только Имя/Роль без outfit notes.\n"
            "  • Описание в скобках используется как fallback для pipeline\n"
            "    если ты забыл вставить [[GEN:type:name:description]] маркер.\n"
            "    Маркер — основной канал, скобки — запасной. Всегда вставляй\n"
            "    маркер, но если забыл — описание в скобках тебя страхует.\n"
            ...
            "  • В `✗ <имя>` строке секции ПЕРСОНАЖИ маркер `[[GEN:...]]`\n"
            "    НЕ ставь — для персонажей пайплайн другой (через вкладку\n"
            "    «Актёры» или «📁 Выбрать существующий»).\n"
            "  • 🔴 ИМЯ ПЕРСОНАЖА в строках `✗`/`✓` — СТРОГО ASCII slug в\n"
            "    нижнем snake_case (имя папки в refs/characters/). Если\n"
            "    в сценарии персонаж называется по-русски/по-украински —\n"
            "    транслитерируй САМ и пиши slug перед оригиналом в\n"
            "    скобках. Примеры:\n"
            "      • «Муж» → `- ✗ muzh (Муж — изменяет жене с подругой) — рефа нет`\n"
            "      • «Жена» → `- ✗ zhena (Жена — узнаёт об измене) — рефа нет`\n"
            "      • «Любовник» → `- ✗ lyubovnik (Любовник — друг семьи) — рефа нет`\n"
            "      • «Чоловік» (укр.) → `- ✗ cholovik (Чоловік — повертається додому) — рефа нет`\n"
            ...
            "  • 🔴 ЖИВОТНЫЕ — НЕ ПЕРСОНАЖИ. Бульдог/кот/лошадь/птица/\n"
            ...
            "      • Хорошо: `- ✗ buldog (Бульдог — сторожевой пёс) — нужен реф [[GEN:object:buldog:Бульдог среднего размера...]]` в ОБЪЕКТАХ\n"
            "      • Плохо: `- ✗ buldog (Бульдог) — рефа нет` в ПЕРСОНАЖАХ\n"
            ...
            "    появляется в разных сценах. Жена в кровати, жена в\n"
            "    халате, жена в куртке — это ВСЁ ОДИН персонаж со\n"
            "    slug'ом `zhena`. Не пиши `zhena_belyy_khalat`,\n"
            "    `zhena_kurtka`, `zhena_uzhinaet` — это ОШИБКА.\n"
            "    Разные одежды/состояния одного героя — это РАЗНЫЕ\n"
            "    ФАЙЛЫ внутри одной папки `refs/characters/zhena/`\n"
            "    (например `zhena_xalat.jpg`, `zhena_kurtka.jpg`), но\n"
            "    slug всегда один. Studio сама подберёт нужный файл\n"
            "    одежды в каждой сцене.\n"
            "  • 🔴 КОГДА упоминать дополнительные одежды героя:\n"
            "    ВСЕГДА когда в сценарии указана ВИЗУАЛЬНО ДРУГАЯ\n"
            "    одежда чем на рефе — будь то смена локации (дома в\n"
            "    халате → на улице в куртке), внутри одной локации\n"
            "    (майка → надевает халат → снимает халат), или\n"
            "    переодевание (был в кашемировом кардигане → стал\n"
            "    в спортивном костюме). Без отдельного outfit-рефа\n"
            "    Nano Banana 2 будет визуально путать одежду или\n"
            "    рисовать её \"по тексту scene_action\" — а в scene_action\n"
            "    одежду описывать словами ЗАПРЕЩЕНО (Правило 6) →\n"
            "    придётся выбирать между двумя нарушениями.\n"
            "    \n"
            "    Решение: упомяни нужный outfit в `✓` строке самого\n"
            "    персонажа отдельным пунктом:\n"
            "    «✓ lora (Лора) — реф есть `lora_xlopkovaya_mayka.jpg`\n"
            "      (для сцен 1-8 — повседневная). Для сцен 9-12 нужна\n"
            "      доп. одежда: **БЕЛЫЙ МАХРОВЫЙ ХАЛАТ** — добавь файл\n"
            "      в `refs/characters/lora/` через вкладку «Актёры»\n"
            "      (нажми «🎨 Сгенерировать» на карточке actress, в\n"
            "      описании укажи `белый махровый халат поверх\n"
            "      хлопковой майки`).»\n"
            "    \n"
            "    ОДИН slug, ОДНА `✓` строка, но с КОНКРЕТНЫМ запросом\n"
            "    дополнительной одежды (если такая нужна). Юзер видит\n"
            "    точно ЧТО ему генерировать, до того как запустить\n"
            "    монтажную карту.\n"
            "    \n"
            "    Если у тебя есть несколько разных одежд для одного\n"
            "    персонажа в эпизоде — перечисли ВСЕ:\n"
            "    «Для сцен 1-3 — кашемировый кардиган (ЕСТЬ),\n"
            "      для сцен 4-7 — рабочий комбинезон (НУЖЕН),\n"
            "      для сцены 8 — пижама (НУЖНА)».\n"
            "    \n"
            "    🔴 ВАЖНО: если юзер скажет «обойдёмся одним рефом, не\n"
            "    хочу генерить ещё одежду» — то Сценарист монтажной\n"
            "    карты ВСЁ РАВНО будет использовать единственный реф\n"
            "    как основу, а упоминание другой одежды (халат, куртка)\n"
            "    в scene_action ЗАПРЕЩЕНО — это видно как баг в\n"
            "    storyboard'е (одежда не совпадает с рефом). См. Правило 6\n"
            "    в storyboard_writer_prompts.py.\n"
            "  • Без manifest вкладка РЕФЕРЕНСЫ в Studio будет ПУСТАЯ для нового эпизода — "
            "юзер не увидит сгенерированные локации\n"
            "  • Если эпизод переиспользует уже существующие рефы (например `prison_visitation` из ep20) — "
            "всё равно перечисли их в manifest текущего эпизода. Это фильтр «что используется ИМЕННО ТУТ».\n"
            ...
            "    (он жмёт кнопки автогенерации рядом с пунктами).\n"
            "  • Каждая `✗` строка ЛОКАЦИИ/ОБЪЕКТА ОБЯЗАТЕЛЬНО содержит\n"
            "    встроенный маркер `[[GEN:type:name:description]]` —\n"
            "    точный формат, примеры и правила в CLAUDE.md ШАГ 1.\n"
            "    Studio парсит маркеры и показывает рядом кнопку\n"
            "    «🎨 Сгенерировать <name>». Юзер кликает → фоновый агент\n"
            "    автономно делает веб-поиск + промпт + pipeline.py +\n"
            "    геометрию. Без маркера — кнопки не будет.\n"
            "  • Для ПЕРСОНАЖА маркер НЕ ставится (другой пайплайн —\n"
            ...
            f"  NOT prose. It is REQUIRED on every `✗` location/object line\n"
            f"  even when your reply is in Ukrainian or English. The token's\n"
