...
class NewEpisodeView(QWidget):
    ...
    def _on_run(self):
        ...
        prompt = (
            ...
            "\n"
            "🔴 ОТБОР В ОБЪЕКТЫ — «значимые предметы», ХОТЯ БЫ ОДИН признак:\n"
            "  1) ДЕТАЛЬНОЕ ОПИСАНИЕ: автор расписал внешность предмета (цвет,\n"
            "     материал, особые черты) → реф даже при одном упоминании.\n"
            "     ✅ «золотые часы с массивным браслетом и гравировкой» → объект.\n"
            "     ❌ «посмотрел на часы» без описания → фон, не объект.\n"
            "  2) ПРЕДМЕТ-ЦЕНТР ДЕЙСТВИЯ: вокруг предмета строится кусок сцены\n"
            "     (транспорт подъезжает / из него выходят; предмет дарят /\n"
            "     передают / разбивают / ставят в вазу) — камера удержит его\n"
            "     в кадре через несколько шотов.\n"
            "     ✅ «чёрный спорткар подъезжает к воротам, из него выходит\n"
            "     женщина» → объект, реф обязателен, даже без описания.\n"
            "     ❌ проходной жест («взглянул на часы», «взял стакан») → фон.\n"
            "  3) ПЕРЕКРЁСТНОСТЬ: предмет упомянут в описании ДРУГОГО референса\n"
            "     (локации / персонажа) → ОБЯЗАН быть заведён и отдельным\n"
            "     объектом. Перед отправкой списка перечитай СВОИ описания\n"
            "     ЛОКАЦИЙ и ПЕРСОНАЖЕЙ и прогони каждый предмет из них по\n"
            "     этому признаку.\n"
            "  4) ПОВТОРЯЕМОСТЬ: предмет в 2 и более сценах → объект (прежний\n"
            "     счётчик — теперь один из признаков, не единственный).\n"
            "  ФОН БЕЗ РЕФА: безымянные группы (гости, толпа, прохожие),\n"
            "  проходные предметы без описания и без сцены вокруг них.\n"
            "\n"
            ...
            "    (значимые предметы), маркер `[[GEN:object:slug:описание]]`.\n"
            ...
            "    (локации, значимые объекты, персонажи) — отдельной строкой.\n"
            ...
            f"    • Russian:    `ЛОКАЦИИ:` / `ОБЪЕКТЫ (значимые предметы):` / `ПЕРСОНАЖИ:`\n"
            f"    • Ukrainian:  `ЛОКАЦІЇ:` / `ОБ'ЄКТИ (значущі предмети):` / `ПЕРСОНАЖІ:`\n"
            f"    • English:    `LOCATIONS:` / `OBJECTS (significant props):` / `CHARACTERS:`\n"
